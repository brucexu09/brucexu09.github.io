...
> 整理自 Day3 复习笔记。面向"白纸能写出来 + 能讲明白"的面试目标,每节给出:**直觉 → 代码骨架 → 数值例子 → 高频追问**。
...
两两算 Jaccard 是 O(N²),百万文档跑不动。

### 核心定理(一句话)

> 对一个随机 hash `h`,**`P(min h(A) == min h(B)) = J(A, B)`**(Jaccard)。

所以把每个文档用 `num_perm` 个独立 hash 各自取最小值 → 得到长度为 `num_perm` 的签名向量。
**两个签名向量逐位相等的比例 = Jaccard 的无偏估计**,只需 O(num_perm) 比较。
...
```
文档 → k-shingles(字符或词 n-gram) → MinHash 签名 → LSH 分桶 → 
桶内两两比签名 → Union-Find 聚类 → 每簇留一篇
```
...
前向: `S = QKᵀ/√D`, `A = softmax(S)`, `out = A·V`。

反向(链式法则一步步推):

```
dV = Aᵀ · grad           # out = A·V → ∂out/∂V = Aᵀ
dA = grad · Vᵀ           # out = A·V → ∂out/∂A 走 V
dS = softmax_backward(A, dA)
dQ = dS · K / √D          # S = QKᵀ/√D
dK = dSᵀ · Q / √D         # 注意转置(S 的列 ↔ K 的行)
```
...
- 人标"这个回答 7 分"非常**不一致**(不同人尺度不同)
- "A 比 B 好" 这种相对判断**稳定得多**
- 所以 RLHF 的 reward model 训在 `(prompt, chosen, rejected)` 三元组上
...
> 对每个 prompt 采 G 个回答,**组内均值当 baseline**,减完归一化得 advantage。loss = `-Σ logπ · A`。
...
> 用 `λ` 在「**低偏差高方差**(蒙特卡洛回报)」和「**高偏差低方差**(一步 TD)」之间插值。
...
- `λ = 1`:  `A_t = Σ_k γ^k · δ_{t+k}` ≈ 蒙特卡洛回报 − value(高方差低偏差)
...
```
Step 1: SFT(监督微调)         → 得到 π_SFT
Step 2: 训 Reward Model        → 得到 RM(用 § 4 的方法)
Step 3: PPO 优化 π             ← 本节
```

### 核心: clipped surrogate
...
```
1. policy   (训练中)
2. ref      (SFT 冻结副本,算 KL 用)
3. reward   (RM 冻结,给分用)
4. value    (actor-critic,算 advantage 用)
```

显存 = 4 倍模型,这是 PPO 工程上的痛点。
...
> **PPO 简化版**: 没有 value 网络,advantage 直接用同 prompt 的 **G 个 rollout 组内归一化 reward**。

### 为什么砍 value?

- value 网络要单独训,和 policy 共享 backbone 又互相干扰
- 数学/代码这类**可验证 reward** 任务,组内归一化已经够好
- 省一个模型 = 省 25% 显存
...
| 显存 | 4 模型 | 3 模型(policy + ref + RM,RM 可换成 verifier) |
...
> 在最优策略下,**最大化 reward + KL 约束** 的解析解可以**反解出 reward**:
> 
> `r(x, y) = β · log(π_θ(y|x) / π_ref(y|x)) + const`

把这个隐式 reward 代回 Bradley-Terry,**reward model 这一步直接消去**,得到一个分类损失:
...
PPO       (prompt) + RM 打分              ✅(独立训)                  ✅(on-policy)
GRPO      (prompt) + RM 或 verifier       ✅ 或 ❌(可验证 reward)      ✅(组内采样)
DPO       (prompt, chosen, rejected)      ❌                          ❌(纯监督)
```

```
策略梯度  →  Reward Model      ──┬──  PPO(+ value)
              ↓                  │
              └──→ 组内归一化  ──┘    GRPO(无 value)
                                      
Bradley-Terry  + Optimal Policy 解析解  →  DPO(无 RM 无 RL)
```
